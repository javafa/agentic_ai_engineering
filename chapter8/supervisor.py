...
@tool
def project_growth(current_value: float, annual_growth_rate: float, years: int) -> str:
    """현재 수치와 연간 성장률로 향후 예상치를 계산합니다. 성장률은 0~1 또는 % 단위 모두 허용합니다."""
    rate = annual_growth_rate / 100 if annual_growth_rate > 1 else annual_growth_rate
    future = current_value * (1 + rate) ** years
    return f"현재 {current_value:g} → {years}년 후 약 {future:,.1f} (연 {rate:.0%} 성장 가정)"
...
def researcher(state: State) -> Command:
    ...
    messages = [
        {"role": "system", "content": (
            "당신은 리서치 전문가입니다. 웹 검색 도구로 사실과 수치를 찾아 '조사 결과'만 정리하세요. "
            "분석이나 예측은 당신의 역할이 아니므로 하지 마세요. "
            "검색은 한두 번이면 충분하며, 같은 주제를 반복 검색하지 마세요. "
            "정보를 모았으면 바로 정리해서 답하세요."
        )},
        *state["messages"],
        # 첫 invoke가 user 턴으로 끝나도록 지시 메시지 추가 (prefill 에러 방지)
    ...
    for _ in range(MAX_TOOL_LOOPS):
        ...
        log("researcher", extract_text(final.content)[:200])
...
def analyst(state: State) -> Command:
    ...
    messages = [
        {"role": "system", "content": (
            "당신은 분석·예측 전문가입니다. 앞선 조사 결과의 수치를 바탕으로 "
    ...
    for _ in range(MAX_TOOL_LOOPS):
        ...
        final = llm.invoke(messages)
        new_messages.append(final)
        log("analyst", extract_text(final.content)[:200])
