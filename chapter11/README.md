...
기본은 헤드리스(창 없음)로 돌아 텍스트 로그만 출력합니다.
`ROBOT_GUI=1` 환경변수를 주면 PyBullet 의 3D 뷰어 창이 열려
로봇이 이동·집기·놓기 하는 모습을 실시간으로 볼 수 있습니다.

```bash
ROBOT_GUI=1 python3 graph.py     # 데모 1건을 뷰어로 관찰
ROBOT_GUI=1 python3 eval.py      # 평가 3건을 연속으로 관찰
```
